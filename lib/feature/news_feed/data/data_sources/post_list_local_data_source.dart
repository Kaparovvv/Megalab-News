import 'dart:convert';

import 'package:megalab_news_app/feature/news_feed/data/models/post_list_model.dart';
import 'package:shared_preferences/shared_preferences.dart';

abstract class PostListLocalDataSource {
  Future<List<PostListModel>> getPostListDataFromCache();
  Future<void> postListDataToCache(List<PostListModel> postListModel);
}

const CACHED_POST_LIST = 'CACHED_POST_LIST';

class PostListLocalDataSourceImpl implements PostListLocalDataSource {
  final SharedPreferences sharedPreferences;

  PostListLocalDataSourceImpl({required this.sharedPreferences});

  @override
  Future<List<PostListModel>> getPostListDataFromCache() {
    final jsonPostList = sharedPreferences.getStringList(CACHED_POST_LIST);
    if (jsonPostList!.isNotEmpty) {
      print('Get Posts from Cache ============  ${jsonPostList.length}');
      return Future.value(
        jsonPostList
            .map((post) => PostListModel.fromJson(json.decode(post)))
            .toList(),
      );
    }
    throw UnimplementedError();
  }

  @override
  Future<void> postListDataToCache(List<PostListModel> postListModel) {
    final List<String> jsonPostList =
        postListModel.map((post) => json.encode(post.toJson())).toList();

    sharedPreferences.setStringList(CACHED_POST_LIST, jsonPostList);
    print('Posts to write cache ============ ${jsonPostList.length}');
    return Future.value(jsonPostList);
  }
}
